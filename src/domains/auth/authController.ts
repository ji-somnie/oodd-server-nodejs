...
router.get('/login/kakao', async (req: Request, res: Response) => {
  ...
  if (!code) {
    return (
      res
        ...
        .json({message: KAKAO_AUTH_FAIL.message, err_code: KAKAO_AUTH_FAIL.err_code})
    );
  }
  ...
  try {
    ...
    return (
      res
        //.status(KAKAO_TOKEN_FAIL.status)
        .json({message: KAKAO_TOKEN_FAIL.message, err_code: KAKAO_TOKEN_FAIL.err_code})
    );
  }
    ...
    if (!username || !kakaoId || !img || !email) {
      return (
        res
          ...
          .json({message: KAKAO_USER_NOT_FOUND.message, err_code: KAKAO_USER_NOT_FOUND.err_code})
      );
    }
  ...
  try {
    ...
    return (
      res
        ...
        .json({message: GOOGLE_TOKEN_FAIL.message, err_code: GOOGLE_TOKEN_FAIL.err_code})
    );
  }
    ...
    if (!username || !googleId || !img || !email) {
      return (
        res
          ...
          .json({message: GOOGLE_USER_NOT_FOUND.message, err_code: GOOGLE_USER_NOT_FOUND.err_code})
      );
    }
...
// 네이버 소셜 로그인 콜백
router.get('/login/naver', async (req: Request, res: Response) => {
  const code = req.query.code as string;
  const state = req.query.state as string;

  if (!code || !state) {
    return res
      .status(status.NAVER_AUTH_FAIL.status)
      .json({message: status.NAVER_AUTH_FAIL.message, err_code: status.NAVER_AUTH_FAIL.err_code});
  }
  ...
  try {
    ...
    return res
      .status(status.NAVER_TOKEN_FAIL.status)
      .json({message: status.NAVER_TOKEN_FAIL.message, err_code: status.NAVER_TOKEN_FAIL.err_code});
  }
