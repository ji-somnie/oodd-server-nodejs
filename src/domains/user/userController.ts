...
userRouter.post('/phone/verification', authenticateJWT, async (req: Request, res: Response) => {
  ...
  if (!phone) {
    return res
      .status(status.PHONE_NUMBER_REQUIRED.status)
      .json({message: status.PHONE_NUMBER_REQUIRED.message, err_code: status.PHONE_NUMBER_REQUIRED.err_code});
  }

  try {
    await userService.sendVerificationCode(phone);
    return res
      .status(status.VERIFICATION_SEND_SUCCESS.status)
      .json({message: status.VERIFICATION_SEND_SUCCESS.message, err_code: status.VERIFICATION_SEND_SUCCESS.err_code});
  } catch (error) {
    return res
      .status(status.VERIFICATION_SEND_FAILED.status)
      .json({message: status.VERIFICATION_SEND_FAILED.message, err_code: status.VERIFICATION_SEND_FAILED.err_code});
  }
});
...
userRouter.post('/phone/verification/check', authenticateJWT, async (req: Request, res: Response) => {
  ...
  if (!phone || !token) {
    return res
      .status(status.VERIFICATION_CODE_NEEDED.status)
      .json({message: status.VERIFICATION_CODE_NEEDED.message, err_code: status.VERIFICATION_CODE_NEEDED.err_code});
  }
  ...
  try {
    ...
    if (isValid) {
      return res
        .status(status.VERIFY_SUCCESS.status)
        .json({message: status.VERIFY_SUCCESS.message, err_code: status.VERIFY_SUCCESS.err_code});
    } else {
      return res
        .status(status.VERIFY_FAILED.status)
        .json({message: status.VERIFY_FAILED.message, err_code: status.VERIFY_FAILED.err_code});
    }
  } catch (error) {
    return res
      .status(status.VERIFY_FAILED.status)
...
userRouter.patch('/:userId/sign-out', authenticateJWT, async (req: Request, res: Response) => {
  try {
    const userId = parseInt(req.params.userId, 10); // Number 대신 parseInt 사용
    const loginedUserId = req.user?.id;
    ...
    if (userId !== loginedUserId) {
      ...
      return res.status(200).json(new BaseResponse(true, HTTP_OK.code, HTTP_OK.message));
    }
  } catch (error) {
    res.status(500).json({message: HTTP_INTERNAL_SERVER_ERROR.message});
  }
