...
import {BasePostListResponseDto, PostDetailResponseDto, PostListResponseDto, PostResponseDto} from './dtos/postResponse.dto';
...
export class PostService {
  ...
  async updatePost(userId: number, postId: number, postRequestDto: PostRequestDto): Promise<BaseResponse<PostResponseDto | null>> {
    try {
      ...
      // 이미지 업데이트 (완전 대체 방식)
    const existingImages = await this.imageRepository.createQueryBuilder('image')
  ...
  // order 값 업데이트
  const orderedImages = await this.imageRepository.createQueryBuilder('image')
    .where('image.postId = :postId', { postId })
  ...
  for (let i = 0; i < orderedImages.length; i++) {
    ...
    await this.imageRepository.save(orderedImages[i]);
      ...
      await this.postStyletagRepository.createQueryBuilder()
        .delete()
      ...
      for (const tag of postRequestDto.styletags || []) {
        let styletag = await this.styletagRepository.createQueryBuilder('styletag')
      ...
      // 옷 정보 업데이트 (완전 대체 방식)
      await this.postClothingRepository.createQueryBuilder()
        .delete()
        .where('postId = :postId', { postId: updatedPost.id })
        .execute();
        ...
        for (const clothingItem of postRequestDto.clothingInfo || []) {
          let clothing = await this.clothingRepository.createQueryBuilder('clothing')
